...
export default function ChamadosUsuarioTable({
  ...
  return (
    <div className="bg-[#1e2128] rounded-lg shadow overflow-hidden">
      <div className="hidden sm:block overflow-x-auto">
        {/* Versão tabela para telas médias/grandes */}
        <table className="min-w-full divide-y divide-[#1e2128] text-xs sm:text-sm md:text-base">
          <thead className="bg-[#17181c]">
            <tr>
              ...
              <th className="px-2 sm:px-4 py-2 text-left font-medium text-gray-400 uppercase cursor-pointer" onClick={() => handleSort('dataCriacao')}>
                Data {renderSortIcon('dataCriacao')}
              </th>
              <th className="px-2 sm:px-4 py-2 text-left font-medium text-gray-400 uppercase">Ações</th>
            </tr>
          </thead>
          <tbody className="bg-[#1e2128] divide-y divide-[rgb(68,68,68)]">
      ...
      {chamados.length === 0 && (
        <div className="p-4 text-center text-gray-600 text-sm">Nenhum chamado encontrado.</div>
      )}
    </div>
  );
}